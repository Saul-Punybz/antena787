...
type TitleMatch struct {
	From  string // como venía en las reglas
	To    string // el nombre bueno, el del catálogo
	Score float64
	Text  string
}
...
// empareje una persona.
type Unmatched struct {
	Name       string
	Candidates []string // los que empataron, si empataron
	Text       string
}
...
func MatchTitles(fromRules, catalog []model.Title) MatchResult {
	res := MatchResult{
	...
	for _, i := range pending {
		...
		if cands := tiedCandidates(newNameKey(t.Name), keys, catalog); len(cands) > 0 {
			u.Candidates = cands
...
func (r *Result) ApplyCatalog(catalog []model.Title) MatchResult {
	m := MatchTitles(r.Titles, catalog)
	old := r.Titles
	r.Titles = m.Titles
...
func tiedCandidates(a nameKey, keys []nameKey, catalog []model.Title) []string {
	best := 0.0
	for _, k := range keys {
	...
	var out []string
	for i, k := range keys {
		if similarity(a, k) >= best-MatchMargin {
...
func newNameKey(name string) nameKey {
	s := strings.ToLower(stripAccents(unescapeMarkdown(name)))
