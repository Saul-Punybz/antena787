...
**7b. Tu VLC, tal cual.** — *Mándanos la cadena exacta con la que emites hoy: el `sout` de VLC, o el archivo `.vlm`/`.xspf` guardado, o el atajo de Windows con los parámetros, y la versión de VLC. Con eso comprobamos, opción por opción, que Antena787 hace todo lo que hoy haces con VLC (destino, multicast, PIDs, códecs, grabación) antes de pedirte que lo apagues.* También: ¿MistServer te empuja la señal o VLC tira de él? ¿Usas la ventana de VLC para ver la salida en el monitor de la torre? (Saul recuerda que MistServer antes te abría una ventana con el video que salía y la última versión ya no: si hoy no tienes forma de ver tu salida, dínoslo, porque Antena787 te la enseña en pantalla y queremos que eso esté desde el primer día.) ¿Alguien más tira de tu señal por HTTP?

**7c. Para tu ingeniero.** — *Hicimos un catálogo de tu equipo (Sage, RVR, el amplificador, el TP1000, la tarjeta receptora) con lo que se puede saber por los manuales públicos. Lo que falta solo lo sabe quien tiene el equipo delante: está en la lista «Preguntas para el ingeniero de CAtv» de `docs/drivers/CATALOGO.md`. Son datos de placa y de menú: modelo exacto, puerto serial libre, pinout del bloque de relés, si el TP1000 recibe unicast o multicast, qué PIDs espera, y si genera PSIP.*
