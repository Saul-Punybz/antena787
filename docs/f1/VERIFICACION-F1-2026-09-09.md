...
| **F1-50** [AUTO] | **PASA** | `internal/importer/f1verif_hoja_test.go:13` `TestF1Verif50DoscientasFilasSieteMalas` | 200 filas, 7 sembradas malas → 193 reglas importadas, 7 `RowErrors` con línea y motivo claro, ninguna con jerga de programador. La hoja nunca se rechaza entera (`internal/importer/rules.go:178`). |
...
| Jerga (*driver*) en un texto en palabras claras | F1-56 | corregido | hecho |
...
- **Dónde se arregla:** `internal/store/schema.sql`, junto a los dos triggers de
  ...
  `ErrOutOfRange` nuevo, en palabras claras.
...
### D-3 · F1-56 — Jerga prohibida en un texto en palabras claras del ingest · **S**
...
`Plainf` es exactamente la máquina de `motivo_claro` (`internal/ingest/ingest.go:49`),
